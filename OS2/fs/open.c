...
PUBLIC int do_open()
{
	int fd = -1;		/* 返回值，判断是否成功打开文件 */

	char pathname[MAX_PATH];

	/*获取消息参数*/
	int flags = fs_msg.FLAGS;	/*存取模式*/
	...
	if (inode_nr == INVALID_INODE) { /*如果文件不存在*/
		if (flags & O_CREAT) {
	...
	if (pin) {
		/*将proc与file_descriptor连接*/
		pcaller->filp[fd] = &f_desc_table[i];

		/*将file_descriptor与inode连接*/
